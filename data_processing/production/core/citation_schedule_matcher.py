...
class CitationScheduleMatcher:
    ...
    def parse_linestring_coordinates(self, linestring: str) -> Optional[List[Tuple[float, float]]]:
        """Parse LINESTRING WKT format to extract GPS coordinates"""
    ...
    def calculate_distance_to_schedule(self, citation_lat: float, citation_lon: float, 
        ...
        """Calculate minimum distance from citation point to schedule line"""
    ...
    def match_citation_to_schedules(self, citation_row: pd.Series, 
        ...
        """Match a single citation to relevant schedules"""
        ...
        # First filter: schedules active on the citation day
        day_column_map = {
        ...
        if citation_weekday in day_column_map:
            day_schedules = schedule_df[schedule_df[day_column_map[citation_weekday]] == 1].copy()
        else:
            return matches
    ...
    def calculate_estimated_citation_times(self, matches_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for schedule_id, group in matches_df.groupby('schedule_id'):
            ...
            stats = {
                ...
                'relevant_citations': len(relevant_matches),
                'avg_citation_hour': np.mean(citation_hours),
                'median_citation_hour': np.median(citation_hours),
                'earliest_citation_hour': np.min(citation_hours),
                'latest_citation_hour': np.max(citation_hours),
                'citation_hour_std': np.std(citation_hours),
                'estimated_sweeper_arrival_hour': np.median(citation_hours) - 0.5,  # Assume citations happen ~30min after sweeper
                'confidence_level': 'HIGH' if len(relevant_matches) >= 10 else 'MEDIUM',
